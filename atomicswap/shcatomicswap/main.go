...
// shareCoinMainNetParams describes Sharecoin's real live mainnet, as
// deployed on the Oracle/GCP nodes (see bitcoin-source/src/kernel/
// chainparams.cpp's CMainParams for the authoritative values this is
// copied from). Sharecoin is a straight Bitcoin Core fork with only the
// PoW algorithm swapped - the wallet RPC surface, script rules, and
// address formats are otherwise untouched upstream Bitcoin, so this tool
// only needs its own address-encoding parameters, not a separate fork of
// btcd itself the way some other altcoins in this repo require.
var shareCoinMainNetParams = chaincfg.Params{
	Name:                    "sharecoinmainnet",
	Net:                     0x53484331, // pchMessageStart "SHC1"
	DefaultPort:             "8443",
	PubKeyHashAddrID:        63,
	ScriptHashAddrID:        18,
	PrivateKeyID:            214,
	WitnessPubKeyHashAddrID: 0, // unused - segwit addresses are bech32-only
	WitnessScriptHashAddrID: 0, // unused - segwit addresses are bech32-only
	Bech32HRPSegwit:         "shc",
...
var (
	chainParams = &shareCoinMainNetParams
)

var (
	flagset     = flag.NewFlagSet("", flag.ExitOnError)
	connectFlag = flagset.String("s", "localhost", "host[:port] of Sharecoin wallet RPC server")
	...
	testnetFlag = flagset.Bool("testnet", false, "not supported - no public Sharecoin testnet is currently deployed")
)
...
func init() {
	flagset.Usage = func() {
		fmt.Println("Usage: shcatomicswap [flags] cmd [cmd args]")
		fmt.Println()
...
func run() (err error, showUsage bool) {
	...
	if *testnetFlag {
		return errors.New("-testnet is not supported: no public Sharecoin " +
			"testnet is currently deployed"), true
	}
...
func walletPort(params *chaincfg.Params) string {
	switch params {
	case &shareCoinMainNetParams:
		return "8332" // matches -rpcport on every deployed Sharecoin mainnet node
	default:
		return ""
	}
}
